...
Consolidado al repo desde ``notebooks/fase_4_homografia/auto_homography.py`` (camino C,
SAM3+YOLO). Las entradas que usa el pipeline del repo son ``solve_masks`` (H sobre
máscara de alfombra externa + centroides de portería) y ``VideoHomography.update_masks``
(lock de arranque + gate de consistencia + EMA + propagación).
"""
...
def _orient_and_build(img, corners, yc, bc) -> AutoH:
    ...
    def goal_err(H):
        ...
        for cc, tgt in ((yc, ft.YELLOW_GOAL_CENTER), (bc, ft.BLUE_GOAL_CENTER)):
            ...
            e += float(np.hypot(p[0] - tgt[0], p[1] - tgt[1]))
            n += 1
