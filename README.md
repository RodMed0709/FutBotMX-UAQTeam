...
![Arquitectura — pipeline de dos capas](assets/readme/png/arquitectura_pipeline.png)

La **Capa A** (GPU) percibe el video y emite el JSON de tracking; la **Capa B** (CPU, solo
cámara superior) lo consume para producir el video de espectador. El **JSON** es el puente
entre ambas: `detector/segmentador` (`sam3_text`/`yolo_sam3`) → `tracker`
(`bytetrack`/`botsort`) → **JSON** → `homografía` (líneas/masks) → `métrica` (cm, velocidad,
zonas, heatmap, Kalman) → `eventos` (gol, posesión, faltas) → `broadcast`.
...
![Capa A — percepción (GPU)](assets/readme/png/capa_a_percepcion.png)
...
![Capa B — post-proceso (CPU)](assets/readme/png/capa_b_postproceso.png)
